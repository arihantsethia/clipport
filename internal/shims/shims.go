...
func renderScript(port int) string {
	...
	fmt.Fprintf(&b, "url=\"http://127.0.0.1:${CLIPPORT_PORT}/v1/clipboard/png\"\n")
	fmt.Fprintf(&b, "if [ -n \"$CLIPPORT_TOKEN\" ] && command -v curl >/dev/null 2>&1 && curl -fsS -H \"Authorization: Bearer ${CLIPPORT_TOKEN}\" \"$url\"; then exit 0; fi\n")
