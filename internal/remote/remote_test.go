...
func TestRemotePathIsDeterministicUnderTmp(t *testing.T) {
	...
	got := RemotePath("dev", "devbox", now)
	want := "/tmp/clipport/dev/devbox/clipboard-20260428-121541.123456.png"
	if got != want {
...
func TestUploaderReturnsRemotePath(t *testing.T) {
	...
	got, err := u.Upload([]byte("png"), "dev", host, route)
	if err != nil {
		t.Fatal(err)
	}
	if got != "/tmp/clipport/dev/devbox/clipboard-20260428-121541.000000.png" {
		t.Fatalf("path = %q", got)
	}
}
